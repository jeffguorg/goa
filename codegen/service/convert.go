...
func ConvertFile(root *expr.RootExpr, service *expr.ServiceExpr) (*codegen.File, error) {
	...
	for _, c := range conversions {
		...
		srcAn := codegen.NewAttributeAnalyzer(
			&expr.AttributeExpr{Type: c.User},
			true, false, true, "", svc.Scope)
		tgtAn := codegen.NewAttributeAnalyzer(
			&expr.AttributeExpr{Type: dt},
			false, false, false, tgtPkg, svc.Scope)
		code, tf, err := codegen.Transform(srcAn, tgtAn, "t", "v", "transform")
		if err != nil {
			return nil, err
		}
	...
	for _, c := range creations {
		...
		srcAn := codegen.NewAttributeAnalyzer(
			&expr.AttributeExpr{Type: dt},
			false, false, false, srcPkg, svc.Scope)
		tgtAn := codegen.NewAttributeAnalyzer(
			&expr.AttributeExpr{Type: c.User},
			true, false, true, "", svc.Scope)
		code, tf, err := codegen.Transform(srcAn, tgtAn, "v", "temp", "transform")
		if err != nil {
			return nil, err
		}
		transFuncs = codegen.AppendHelpers(transFuncs, tf)
